...
@Service
public class AsyncHrConsumerService {
	private final WebClient webClient;
	
	public AsyncHrConsumerService(WebClient webClient) {
	...
	public void callHrRestApi() {
		...
		hireEmployeeRequest.setIban("DE06700500000005573241");
	    ...
	    webClient.post()
                 ...
                 .doOnNext( hireResponse -> {
                	System.out.println(hireResponse);
                	
                })
                .doOnError(error -> System.err.println(error))
                .subscribe(_ ->{
                	webClient.delete()
			       	         .uri("/".formatted(identityNo))
			       	         .retrieve()
			       	         .bodyToMono(String.class)
			       	         .doOnNext(fireResponse -> {
			       	        	 System.out.println(fireResponse);
			       	         })
			       	         .doOnError(error -> System.err.println(error))
			       	         .subscribe();
                });
