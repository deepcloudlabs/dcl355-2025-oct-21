...
import com.example.hr.client.dto.request.HireEmployeeRequest;
...
public class HrConsumerService {
	...
	public HrConsumerService(RestTemplate restTemplate, 
			@Value("${hrRestApiUrl}") String hrRestApiUrl) {
		this.restTemplate = restTemplate;
		this.hrRestApiUrl = hrRestApiUrl;
	}

	@Scheduled(fixedRateString = "${callRate}")
	public void callHrRestApi() {
		var hireEmployeeRequest = new HireEmployeeRequest();
		String identityNo = "88348727490";
		hireEmployeeRequest.setIdentityNo(identityNo);
		hireEmployeeRequest.setFirstName("kate");
		hireEmployeeRequest.setLastName("auten");
		hireEmployeeRequest.setSalary(300_000);
		hireEmployeeRequest.setCurrency("USD");
		hireEmployeeRequest.setJobStyle("FULL_TIME");
		hireEmployeeRequest.setIban("DE05593501101370793364");
		hireEmployeeRequest.setEmail("kate@example.com");
		hireEmployeeRequest.setDepartments(List.of("IT","SALES"));
		...
		hireEmployeeRequest.setBirthYear(1988);
		var hireResponse = restTemplate.postForEntity(hrRestApiUrl, hireEmployeeRequest, String.class);
		System.err.println(hireResponse.getBody());        
		 restTemplate.delete(hrRestApiUrl.concat("/%s".formatted(identityNo)));
	}
}
